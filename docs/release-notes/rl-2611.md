...
- {file}`hm-session-vars.sh` is now safe to apply more than once, so a new
  shell picks up values changed by `home-manager switch` without logging out.
  Plain variables are re-exported, and search variables add only the entries
  that are missing. [](#opt-home.sessionVariablesExtra) keeps its
  once-per-session guard, because it holds arbitrary code. Interactive
  non-login Bash shells follow in a separate change.

  Two consequences. A value that references the variable it defines now
  accumulates copies; Home Manager warns about the cases it can detect, and
  the search-path options are the supported replacement. And removing a
  variable still does not unset it in an existing session, because nothing
  records what a previous generation exported; that continues to need a new
  session.
